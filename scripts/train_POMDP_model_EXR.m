function [mdp, Z] = train_POMDP_model_EXR(p)
% 
...
for i = subjects
    ...
    Y = func_build_Y_from_data_EXR(df);
    ...
    Z.Cp = Cp;
    Z.F = Fvec;
    save('../results/POMDP estimates/Z_EXR_allSubj','Z')
end
...
function [Y] = func_build_Y_from_data_EXR(df)
n_stim_per_block = 24;
n_blocks = 8;

% Delete non-EXR trials
df(df.effort == "RLX",:) = [];
